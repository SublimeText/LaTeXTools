...
class ChromeViewer(BaseViewer):
    ...
    # def _test_chrome_view():

    #     def get_browser_filepath(tex_line_no,tex_col_no,tex_filename,pdf_filename,zoom=100):

    #         try:

    #             output_text = sp.run(['synctex', 'view', '-i', f'{tex_line_no}:{tex_col_no}:{tex_filename}', '-o', pdf_filename],capture_output=True,text=True).stdout.split("\n")
    ...
    #         return f"file://{pdf_filename}#page={page_no}&view=FitH,{line_no}"



    #     def run_applescript(tex_line_no,tex_col_no,tex_filename,zoom=100):

    #         pdf_filename = tex_filename.rsplit(".",1)[0] + ".pdf"

    #         finalDestination = get_browser_filepath(tex_line_no,tex_col_no,tex_filename,pdf_filename)

    #         quotedPDFName = quote(pdf_filename)

    #         defaultCmd = "/Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome '{finalDestination}'"

    #         result = sp.run(['osascript', "/Users/zach/Library/Application Support/Sublime Text/Packages/LaTeXTools/viewers/OpenFileInChrome_Page_Number.applescript", finalDestination,quotedPDFName,defaultCmd],capture_output=True,text=True).stdout
    ...
    #         tex_filename = "/Users/zach/Documents/UCLA_classes/stats203/project/final/final.tex"


    #         run_applescript(tex_line_no,tex_col_no,tex_filename,zoom=100)
